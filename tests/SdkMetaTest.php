...
class SdkMetaTest extends TestCase
{
    ...
    {
        $this->assertSame('SOM-096', $this->meta['app_id']);
    }
    ...
    public function test_endpoints_has_manager_send_by_template(): void
    {
        $endpoints = $this->meta['endpoints'];
        $this->assertArrayHasKey('manager', $endpoints);
        $this->assertArrayHasKey('send_by_template', $endpoints['manager']);
        $this->assertNotEmpty($endpoints['manager']['send_by_template']);
    }
